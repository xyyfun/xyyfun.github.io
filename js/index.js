window.addEventListener('load', function () {
	...
	// banner左侧快捷访问链接
	var category_li = this.document.querySelector('.category').querySelector('ul').querySelectorAll('li');
	for (var i = 0; i < category_li.length; i++) {
		category_li[i].addEventListener('mouseenter', function () {
			for (var i = 0; i < category_li.length; i++) {
				category_li[i].style.backgroundColor = '';
			}
			this.style.backgroundColor = '#39ba9b';
		});
		category_li[i].addEventListener('mouseleave', function () {
			for (var i = 0; i < category_li.length; i++) {
				category_li[i].style.backgroundColor = '';
			}
		});
